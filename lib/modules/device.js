...
function OnvifDevice(params) {
	...
	this.onvifHttpAuth = new OnvifHttpAuth();
	this.address = '';
	this.xaddr = '';
	this.user = '';
	this.pass = '';
...
OnvifDevice.prototype.fetchSnapshot = function (callback) {
	let promise = new Promise((resolve, reject) => {
		...
		let req = this.onvifHttpAuth.request(options, (res) => {
			let buffer_list = [];
			res.on('data', (buf) => {
...
OnvifDevice.prototype._getCapabilities = function () {
	let promise = new Promise((resolve, reject) => {
		this.services.device.getCapabilities((error, result) => {
			...
			let recording = c['Extension'] && c['Extension']['Recording'];
			if (recording && recording['XAddr']) {
				this.services.recording = new mOnvifServiceRecording({
					'xaddr': this._getXaddr(recording['XAddr']),
					'time_diff': this.time_diff,
					'user': this.user,
					'pass': this.pass
				});
			...
			profiles.forEach((p) => {
				...
				if (p['VideoEncoderConfiguration']) {
					profile['video']['encoder'] = {
						...
						'resolution': {
							'width': p['VideoEncoderConfiguration']['Resolution'] ? parseInt(p['VideoEncoderConfiguration']['Resolution']['Width'], 10) : 0,
							'height': p['VideoEncoderConfiguration']['Resolution'] ? parseInt(p['VideoEncoderConfiguration']['Resolution']['Height'], 10) : 0,
						},
						'quality': parseInt(p['VideoEncoderConfiguration']['Quality'], 10),
						'framerate': p['VideoEncoderConfiguration']['RateControl'] ? parseInt(p['VideoEncoderConfiguration']['RateControl']['FrameRateLimit'], 10) : 0,
						'bitrate': p['VideoEncoderConfiguration']['RateControl'] ? parseInt(p['VideoEncoderConfiguration']['RateControl']['BitrateLimit'], 10) : 0,
						'encoding': p['VideoEncoderConfiguration']['Encoding']
					};
					if( p['VideoEncoderConfiguration']['H264'])
						profile['video']['encoder'].gov = parseInt(p['VideoEncoderConfiguration']['H264']['GovLength'],10);
				}
