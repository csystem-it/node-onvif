...
// * - params:
// *   - Configuration | String | required | xml string of video encoder configuratio
// *   - ForcePersistence | boolean | required | the changes shall remain after reboot of the device or not
// *
// * {
// *   empty
// * }
...
// 		// if(err_msg = mOnvifSoap.isInvalidValue(params['Configuration'], 'string')) {
...
// 			reject(new Error('The "ForcePersistence" property was invalid: ' + err_msg));
// 			return;
// 		}
	
// 		let soap_body = '';
// 		soap_body += '<trt:SetVideoEncoderConfiguration>';
// 		soap_body +=   params['Configuration'];
// 		soap_body +=   '<trt:ForcePersistence>' + params['ForcePersistence'] + '</trt:ForcePersistence>';
// 		soap_body += '</trt:SetVideoEncoderConfiguration>';
// 		let soap = this._createRequestSoap(soap_body);
...
/* ------------------------------------------------------------------
* Method: sendRequest(soap_body[, callback])
* - params:
*   - methodName | name of the method to send
*   - soap_body | body of request to send
* {
*   result of the request
* }
* ---------------------------------------------------------------- */
OnvifServiceMedia.prototype.sendRequest = function(methodName, soap_body, callback) {
	let promise = new Promise((resolve, reject) => {
		
		let soap = this._createRequestSoap(soap_body);
	
		mOnvifSoap.requestCommand(this.oxaddr, methodName, soap).then((result) => {
			resolve(result);
		}).catch((error) => {
			reject(error);
		});
